    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.distributors.update", [$distributor->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $distributor->distributor->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('username') ? 'is-invalid' : '' }}" type="text" name="username" id="username" value="{{ old('username', $distributor->distributor->username) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('roles') ? 'is-invalid' : '' }}" name="roles[]" id="roles" multiple required>
                    @foreach($roles as $id => $roles)
                        <option value="{{ $id }}" {{ (in_array($id, old('roles', [])) || $distributor->distributor->roles->contains($id)) ? 'selected' : '' }}>{{ $roles }}</option>
                    @endforeach
                </select>
                @if($errors->has('roles'))
                    <div class="invalid-feedback">
            ...
            <!--<div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('terminals') ? 'is-invalid' : '' }}" name="terminals[]" id="terminals" multiple>
                    @foreach($terminals as $id => $terminal)
                        <option value="{{ $id }}" {{ (in_array($id, old('terminals', [])) || $distributor->distributor->terminals->contains($id)) ? 'selected' : '' }}>{{ $id .' - '.$terminal }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ships') ? 'is-invalid' : '' }}" name="ships[]" id="terminals" multiple >
                    ...
                    @foreach($ships as $ship)
                            <option
                                value="{{ $ship->id }}" {{ (in_array($ship->id, old('ships', [])) || $distributor->distributor->ships->contains($ship->id)) ? 'selected' : '' }}>{{ $ship->ship_ids.' - '.$ship->name }}</option>
                    @endforeach
                </select>
                @if($errors->has('ships'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <?php $i = 0; ?>
                @if (count($distributor->distributor->email) != "")
                @foreach($distributor->distributor->email as $email)
